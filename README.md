...
This is the fresh implementation in `gary-app`, version **0.3.0**. It preserves the previous Toby app’s dark, editorial direction while replacing its architecture. No permanent conversation sidebar. No data migration from the old app.
...
- Native SwiftUI workspace, soft rounded headings, warm charcoal/cream/apricot palette, library rows, menu bar and in-thread voice capture. Settings opens as an animated in-app side drawer (also Command–Comma); recording stays in the workspace.
