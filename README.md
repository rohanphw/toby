...
This is the fresh implementation in `gary-app`, version **0.4.0**. It preserves the previous Toby app’s dark, editorial direction while replacing its architecture. No permanent conversation sidebar. No data migration from the old app.
...
1. Open the app yourself. Settings → Intelligence → Check CLI session reuses the existing login for each provider. If needed, run `codex login` or `grok login` in Terminal once, then check again. Select Codex or Grok for subsequent tasks. Use Load models on Home or Check CLI session in Settings to discover that provider’s available models and save a default. Choose executable is available when discovery fails.
...
- Native SwiftUI workspace, soft rounded headings, glossy black surfaces and silver accents, library rows, menu bar and in-thread voice capture. Settings opens as an animated in-app side drawer (also Command–Comma); recording stays in the workspace.
...
- Both CLIs own authentication and may use their existing local configuration. Toby never reads credential files, copies tokens or asks for an API key. Grok explicitly authenticates with its cached CLI token; no API-key fallback is selected by Toby. Grok uses the selected model (or its CLI default) and native tools; permission requests are presented for one-time approval. Executable discovery supports standard installs and NVM; Settings can select a custom executable.
