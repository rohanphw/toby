...
- Explicit model loading calls `_x.ai/models/list`. ACP adds the leading underscore for extension methods; the Rust handler is named `x.ai/models/list`.
...
- Save the choice under `grokModel`, separately from `codexModel`. An empty choice means CLI default.
